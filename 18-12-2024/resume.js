...
//skills//hobbies//language
function deleteLi(value,key,id) {
  alert("okay")
  let temp = [];
  ...
  resume[key] = temp;
  displayArrayValue(key,id)
}
window.deleteLi = deleteLi;


//skills//hobbies//language
function displayArrayValue(key, id) {
  let ul = "";
  for (let each of resume[key]) {
    console.log( resume[key])
    ul = ul + `<li>${each}
                <button onclick="deleteLi('${each}','${key}','${id}')">delete</button>
                </li>`
...
//                      <button onclick="deleteEdu('${i}')">delete</button>    
//    </tr>`
// }
...
function displayList() {
  let trs = "";
  // for (let each of resume.education_details) {
  let edu = resume.education_details;
  for(let i in edu) {
    //console.log(edu[i])
    trs = trs + `<tr>
                     <td>${edu[i].course_name}</td>
                     <td> ${edu[i].course_institute}</td>
                     <td> ${edu[i].course_year}</td>
                     <td> ${edu[i].course_percentage}</td>
                     <button onclick="deleteEdu('${i}','education_details')">delete</button>    
                      </tr>`
  }
  //}
...
function displayWork() {
  ...
  for (let each of resume.work_experience) {
    ...
    trs = trs + `<tr>
                     ...
                     <td> ${each.duration}</td>
                     <td> ${each.year}</td>
                    <button onclick="deleteWork('${each.company_name}')">delete</button>
                      </tr>`
...
//certification
function deleteCertification(value) {
  alert("done")
  let x = [];
...
function displayCertification() {
  ...
  for (let each of resume.certification) {
    ...
    trs = trs + `<tr>
                     ...
                     <td> ${each.year1}</td>
                      <button onclick="deleteCertification('${each.year1}')">delete</button>
                      </tr>`
...
function displayProject() {
  ...
  for (let each of resume.project) {
    ...
    trs = trs + `<tr>
                     ...
                     <td> ${each.year2}</td>
                     <button onclick="deleteProject('${each.title}')">delete</button>
...
function editResume(id, data) {
  ...
  // displayList()
  displayArrayValue("skills", "skill")


  let hobbies = JSON.parse(data.hobbies);
  resume.hobbies = hobbies;
  displayArrayValue("hobbies", "hby")
  //displayList1()
  ...
  displayArrayValue("language_known", "language")
